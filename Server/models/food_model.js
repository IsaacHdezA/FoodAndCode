...
food_model.new_food = (datos, cb) => {
  conn.query(
    "INSERT INTO comida(com_cat_id, com_nombre, com_precio, com_descripcion) VALUES (?,?,?,?);",
    datos,
    cb
  );
};
...
food_model.all_foods = (datos, cb) => {
  conn.query(
    "SELECT co.*, ca.cat_nombre " +
      "FROM comida co " +
      "INNER JOIN categoria ca ON co.com_cat_id = ca.cat_id ORDER BY co.com_nombre " +
      ";",
    [],
    cb
  );
...
food_model.update_food = (datos, cb) => {
  conn.query(
    "UPDATE comida SET com_cat_id = ?, com_nombre = ?, com_precio = ?, com_descripcion = ? " +
      "WHERE com_id = ?;",
    datos,
    cb
  );
